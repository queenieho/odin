...
(defn approve-modal
  [account application]
  (let [is-showing   (subscribe [:modal/visible? :accounts.approval/modal])
        units        (subscribe [:accounts.entry.approval/units])
        is-approving (subscribe [:ui/loading? :accounts.entry/approve])
     ...
     {:component-will-mount
      (fn [_]
        (when-some [p (:community @form)]
          (dispatch [:accounts.entry.approval/fetch-units p])))
      :reagent-render
      (fn [account]
        [ant/modal
         {:title     (str "Approve " (:name account))
          :visible   @is-showing
          :on-cancel #(dispatch [:modal/hide :accounts.approval/modal])
          :footer    [(r/as-element
                       ^{:key 1}
                       [ant/button
                        {:on-click #(dispatch [:modal/hide :accounts.approval/modal])
                         :size     :large}
                        "Cancel"])
                      (r/as-element
                       ^{:key 2}
                       [ant/button
                        {:disabled (-> @form :unit nil?)
                         ...
                         :on-click #(dispatch [:accounts.entry/approve (:id application) @form])}
                        "Approve"])]}
         ;; Choose community
         [ant/form-item {:label "Approve for which community?"}
          [ant/radio-group
           {:on-change #(let [property-id (.. % -target -value)]
                          ...
                          (dispatch [:accounts.entry.approval/fetch-units property-id]))
    ...
    (cond
      ...
      [ant/button
       {:size     :large
        ...
        :on-click #(dispatch [:modal/show :accounts.approval/modal])}
       "Approve"]
